...
N    =  6 if len(sys.argv) < 4 else int(sys.argv[3])
NGQB =  3 if len(sys.argv) < 5 else int(sys.argv[4])
NSQB =  6 if len(sys.argv) < 6 else int(sys.argv[5])
NLQB =  3 if len(sys.argv) < 7 else int(sys.argv[6])

print_header(N, NGQB, NSQB, NLQB, True)
simple_test("first test", True, sys.argv[1], sys.argv[2], N, NGQB)
print("===========================")
