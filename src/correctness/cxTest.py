...
class cxTest:
    ...
    def _test_cx(self, name, x_range,y_range):
        flag = True
        for x in x_range:
            for y in y_range:
                if(x==y):continue
    ...
    def test(self):
        ...
        for i in range(4):
            for j in range(4):
                ...
                flag = self._test_cx(test_name, self.range_type[i], self.range_type[j])
                if not flag:    break
            if not flag:    break
        if(flag):
            print("[PASS]", "CX Test", ": match with qiskit under 1e-9", flush=True)
            print("===========================")
        else:
            print("[x]", "CX Test", ": not pass under 1e-9", flush=True)
            print("===========================")
            return
