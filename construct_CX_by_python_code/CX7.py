...
def CX(ctrl, targ, fd_arr, one_gate):
    ...
    if isLocal(ctrl) and isLocal(targ):
        offsets = [large_offset, small_offset, half_ctrl_offset, half_targ_offset]
        # [Thread level parallel] here
        for fd in fd_arr:
            for j in range(0, FILESIZE, CHUNKSIZE):
                work = []
...
def test(name:str, r1, r2, CTrev:bool, gate_func, h_list):
    ...
    if(flag):
        print("[pass]", name, ": match with qiskit under 1e-9")
    else:
        # print_state_f(fd_arr)
        # print_state(state)
        print("[x]", name, "not pass")
...
for h in range(N):
    ...
    test("Thread test", range(NGQB, NSQB), range(NGQB, NSQB), False, x_gate, h_list)
    test("Ctrl-Thread Targ-Middle test", range(NGQB, NSQB), range(NSQB, N-NLQB), False, x_gate, h_list)
    test("Targ-Thread Ctrl-Middle test", range(NGQB, NSQB), range(NSQB, N-NLQB), True, x_gate, h_list)
    print("===========================")
